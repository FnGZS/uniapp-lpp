<template>
	<view id='orderDetail'>
		<view class='orderStatus'>
			<!-- <view class='orderStatusText' v-if="orderDetail.orderState==1">取消预约</view> -->
			<view class='orderStatusText' v-if="orderDetail.orderState==1">预约中</view>
			<view class='orderStatusText' v-if="orderDetail.orderState==2">预约成功</view>
			<view class='orderStatusText' v-if="orderDetail.orderState==3">预约失败</view>
			<view class='orderStatusText' v-if="orderDetail.orderState==4">已关闭</view>
		</view>
		<view class='orderAddress'>
			<view class='iconfont icon-dizhi'></view>
			<view class='orderAddress-main'>
				<text class='orderAddress-main-name'>{{orderDetail.employerName}}</text>
				<text class='orderAddress-main-phone'>{{orderDetail.employerPhone}}</text>
				<view class='orderAddress-main-pos'>{{orderDetail.address}}</view>
			</view>
		</view>
		<view class="goods-list">
			<view class="list-title">基础信息 </view>
			<view class="a-goods" key="index">
				<view class="img-box">
					<image :src="orderDetail.picUrl" mode='aspectFill' class="img" />
				</view>
				<view class="text-box">
					<view class="arow arow01">
						<view class="goods-name">{{orderDetail.cleaner.name}}</view>
						<view class="goods-price">¥ {{orderDetail.cleaner.price}}</view>
					</view>
					<view class="arow">
						<view class="goods-label">{{orderDetail.cleaner.card}}</view>
						<view class="goods-num">x 1</view>
					</view>
				</view>
			</view>
			<view class='orderNum'>
				<view class='orderNum-text' >服务范围</view>
				<view class='orderPrice-num'>{{orderDetail.cleaner.place}}</view>
			</view>
			
			<view class='orderPrice'>
				<view class='orderPrice-text'>订单总价</view>
				<view class='orderPrice-num'>￥{{orderDetail.cleaner.price}}</view>
			</view>
			
			
		</view>
		<view class='orderMes'>
			<view class='orderMes-title'>订单信息</view>
			<view class='orderNum'>
				<view class='orderNum-Text'>订单编号</view>
				<view class='orderNum-number'>{{orderDetail.id}}</view>
			</view>
			<view class='orderTime'>
				<view class='orderTime-Text'>下单时间</view>
				<view class='orderTime-number'>{{orderDetail.gmtCreated}}</view>
			</view>
			<!-- <view class='orderPayWay'>
				<view class='orderPayWay-Text'>支付方式</view>
				<view class='orderPayWay-number'>在线支付</view>
			</view> -->
		</view>
		<view class='barBottom'>
			<view class='orderStatusBtn'>
				<button class="cu-btn block line-grey " v-if="orderDetail.orderState==1">取消预约</button>
				<button class="cu-btn block line-blue " v-if="orderDetail.orderState==1">预约中</button>
				<button class="cu-btn block line-cyan "  v-if="orderDetail.orderState==2">预约成功</button>
				<button class="cu-btn block line-gray "  v-if="orderDetail.orderState==3">预约失败</button>
				<button class="cu-btn block line-grey "  v-if="orderDetail.orderState==4">已关闭</button>
				<!-- <button class="cu-btn block line-grey sm" v-if="orderDetail.goodsState==0 && orderDetail.fuwuState == 2">取消订单</button>
				<button class="cu-btn block line-red sm"  v-if="orderDetail.goodsState==0 && orderDetail.fuwuState == 2">立即支付</button>
				<button class="cu-btn block line-gray sm" v-if="orderDetail.goodsState==0 && orderDetail.fuwuState == 0">预约失败</button>
				<button class="cu-btn block line-blue sm" v-if="orderDetail.goodsState==0 && orderDetail.fuwuState == 1">预约中</button>
				<button class="cu-btn block line-cyan sm"  v-if="orderDetail.goodsState==1 && orderDetail.fuwuState == 2">待服务</button>
				<button class="cu-btn block line-green sm"  v-if="orderDetail.goodsState==1 && orderDetail.fuwuState == 3">服务中</button>
				<button class="cu-btn block line-orange sm"  v-if="orderDetail.goodsState==1 && orderDetail.fuwuState == 4">去评价</button>
				<button class="cu-btn block line-pink sm"  v-if="orderDetail.goodsState==1 && orderDetail.fuwuState ==4">付尾款</button>
				<button class="cu-btn block line-pink sm"  v-if="orderDetail.goodsState==1 && orderDetail.fuwuState ==5">已评价</button> -->
			</view>
		</view>
	</view>
</template>

<script>
	import { sendAjax } from '@/common/js/sendAjax.js';
	import config from '@/apiConfig';
	const { getOrderDetail} = config.api;
	export default {
		data() {
			return {
				isUser:1,
				id:null,
				orderDetail:{}
			}
		},
		onLoad(option) {
			console.log(option.id)
			this.id = option.id;
			this.getOrderDetail()
		},
		methods: {
			
			getOrderDetail(){
				var that = this;
				var id = this.id;
				let infoOpt = {
					url: getOrderDetail  + id,
					type: 'GET',
					data:{}
				};
				let infoCb = {};
				infoCb.success = function(res) {
					console.log(res)
					that.orderDetail = res;
					uni.hideLoading()
				},
				infoCb.beforeSend = () => {
				  uni.showLoading({
				  	title:'加载中'
				  })
				};
				sendAjax(infoOpt, infoCb);
			}
		}
	}
</script>

<style>
	@font-face {
		font-family: "iconfont";
		src: url('https://at.alicdn.com/t/iconfont.eot?t=1545902147596');
		src: url('https://at.alicdn.com/t/iconfont.eot?t=1545902147596#iefix') format('embedded-opentype'),
			url('data:application/x-font-woff;charset=utf-8;base64,') format('woff'),
			url('https://at.alicdn.com/t/iconfont.ttf?t=1545902147596') format('truetype'),
			url('https://at.alicdn.com/t/iconfont.svg?t=1545902147596#iconfont') format('svg');
	}

	.iconfont {
		font-family: "iconfont" !important;
		font-size: 16px;
		font-style: normal;
		-webkit-font-smoothing: antialiased;
		-moz-osx-font-smoothing: grayscale;
	}

	.icon-dizhi:before {
		content: "\e652";
	}

	page {
		min-height: 100%;
		background-color: #f5f5f5;
	}

	#orderDetail {
		width: 100%;
		height: 100%;
	}

	.orderStatus {
		background: linear-gradient(to bottom ,#3598DC, #63B8FF);
		background-size: 100% 100%;
		width: 100%;
		height: 200upx;
		position: relative;
	}

	.orderStatusText {
		margin-left: 5%;
		color: #fff;
		height: 50upx;
		font-size: 30upx;
		line-height: 200upx;
	}

	.orderGoods {
		margin-top: 20upx;
		background: #fff;
	}

	.orderGoods-title {
		margin-left: 5%;
		font-size: 28upx;
		font-weight: bold;
		color: #585858;
		line-height: 80upx;
		border-bottom: 1px solid #f5f5f;
	}

	.orderAddress {
		display: flex;
		padding-bottom: 10upx;
		padding-top: 10upx;
		background: #fff;
	}

	.icon-dizhi {
		color: #3598dc;
		line-height: 100upx;
		font-size: 50upx;
		margin-left: 5%;
		margin-right: 20upx;
	}

	.orderAddress-main {
		width: 80%;
	}

	.orderAddress-main-name {
		font-size: 28upx;
		margin-right: 20upx;
	}

	.orderAddress-main-phone {
		font-size: 25upx;
		color: #a8a8a8;
	}

	.orderAddress-main-pos {
		font-size: 28upx;
	}

	.goods-list {
		width: 100%;
		margin-top: 20upx;
		background-color: #fff;
		border-bottom: 10upx solid #f5f5f5;
	}

	.goods-list .list-title {
		font-size: 28upx;
		color: #585858;
		font-weight: bold;
		margin-left: 5%;
		line-height: 80upx;
	}

	.goods-list .a-goods {
		width: 90%;
		margin-left: 5%;
		display: flex;
		/*justify-content: space-between;*/
		border-top: 1px solid #eee;
	}

	.goods-list .a-goods .img-box {
		width: 160upx;
		height: 160upx;
		overflow: hidden;
		margin-top: 20upx;
		margin-right: 20upx;
		background-color: #d8d8d8;
	}

	.goods-list .img-box .img {
		width: 160upx;
		height: 160upx;
	}

	.goods-list .a-goods .text-box {
		width: 510upx;
		box-sizing: border-box;
		padding-top: 10upx;
	}

	.a-goods .text-box .arow {
		display: flex;
		justify-content: space-between;
		align-items: center;
	}

	.a-goods .text-box .arow .goods-name {
		width: 360upx;
		font-size: 26upx;
		height: 74upx;
		color: #000;
		line-height: 1.6;
		overflow: hidden;
		text-overflow: ellipsis;
		display: -webkit-box;
		-webkit-box-orient: vertical;
		-webkit-line-clamp: 2;
	}

	.a-goods .text-box .arow .goods-price {
		font-size: 26upx;
		color: #000;
		align-self: flex-start;
	}

	.a-goods .text-box .arow .goods-label {
		font-size: 26upx;
		margin-top: 30upx;
		width: 70%;
		color: #999;
		overflow: hidden;
		text-overflow: ellipsis;
		display: -webkit-box;
		-webkit-box-orient: vertical;
		-webkit-line-clamp: 1;
	}

	.a-goods .text-box .arow .goods-num {
		font-size: 26upx;
		color: #999;
	}

	.orderPrice {
		width: 100%;
		font-size: 28upx;
		position: relative;
		line-height: 80upx;
	}

	.orderPrice-text {
		margin-left: 5%;
	}

	.orderPrice-num {
		position: absolute;
		top: 0upx;
		right: 5%;
	}

	.orderPayPrice {
		width: 100%;
		font-size: 28upx;
		position: relative;
		line-height: 80upx;
	}

	.orderPayPrice-text {
		border-top: 1upx solid #f5f5f5;
		margin-left: 5%;
	}

	.orderPayPrice-num {
		position: absolute;
		top: 0upx;
		right: 5%;
		color: #ef696c;
	}

	.orderMes {
		background: #fff;
		margin-top: 20upx;
	}

	.orderMes-title {
		font-size: 28upx;
		color: #585858;
		font-weight: bold;
		margin-left: 5%;
		line-height: 80upx;
		border-bottom: 1px solid #f5f5f5;
	}

	.orderNum {
		margin-left: 5%;
		display: flex;
		font-size: 28upx;
		border-bottom: 1px solid #f5f5f5;
		height: 70upx;
		line-height: 70upx;
		position: relative;
	}

	.orderNum-number {
		position: absolute;
		top: 0upx;
		right: 5%;
	}

	.orderTime {
		margin-left: 5%;
		display: flex;
		font-size: 28upx;
		height: 70upx;
		line-height: 70upx;
		border-bottom: 1px solid #f5f5f5;
		position: relative;
	}

	.orderTime-number {
		position: absolute;
		top: 0upx;
		right: 5%;
	}

	.orderPayWay {
		margin-left: 5%;
		display: flex;
		font-size: 28upx;
		height: 70upx;
		line-height: 70upx;
		border-bottom: 1px solid #f5f5f5;
		position: relative;
	}

	.orderPayWay-number {
		position: absolute;
		top: 0upx;
		right: 5%;
	}

	.orderWay {
		margin-left: 5%;
		display: flex;
		font-size: 28upx;
		height: 70upx;
		line-height: 70upx;
		border-bottom: 1px solid #f5f5f5;
		position: relative;
		margin-bottom: 120upx;
	}

	.orderPayWay-number {
		position: absolute;
		top: 0upx;
		right: 5%;
	}

	.barBottom {
		height: 90upx;
		position: fixed;
		bottom: 0;
		width: 100%;
		background: #fff;
		border-top: 1upx solid #ddd;
	}

	.orderStatusBtn {
		position: absolute;
		right: 5%;
		top: 10upx;
		display: flex;
		font-size: 25upx;
		height: 70upx;
	}
	.orderStatusBtn button{
		margin-left: 10upx;
	}
	

	
</style>
